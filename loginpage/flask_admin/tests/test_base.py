...
def test_base_defaults():
    ...
    eq_(admin.url, '/admin1')
    eq_(admin.endpoint, 'admin1')
    eq_(admin.app, None)
    ok_(admin.index_view is not None)
    eq_(admin.index_view._template, 'admin1/index.html')
...
def test_admin_customizations():
    app = Flask(__name__)
    admin = base.Admin(app, name='Test', url='/foobar', static_url_path='/static/my/admin1')
    eq_(admin.name, 'Test')
    eq_(admin.url, '/foobar')
    eq_(admin.index_view.blueprint.static_url_path, '/static/my/admin1')
    ...
    with app.test_request_context('/'):
        rv = client.get(url_for('admin1.static', filename='bootstrap/bootstrap2/css/bootstrap.css'))
    eq_(rv.status_code, 200)
...
def test_baseview_registration():
    ...
    eq_(view.url, '/admin1/foobar')

    view = MockView(url='test')
    view.create_blueprint(base.Admin())
    eq_(view.url, '/admin1/test')
...
def test_call():
    ...
    rv = client.get('/admin1/')
    eq_(rv.status_code, 200)

    rv = client.get('/admin1/mockview/')
    eq_(rv.data, b'Success!')

    rv = client.get('/admin1/mockview/test/')
    eq_(rv.data, b'Success!')
...
def test_nested_flask_views():
    ...
    rv = client.get('/admin1/mockmethodview/_api/1')
    print('"', rv.data, '"')
    eq_(rv.data, b'GET - API1')
    rv = client.put('/admin1/mockmethodview/_api/1')
    eq_(rv.data, b'PUT - API1')
    rv = client.post('/admin1/mockmethodview/_api/1')
    eq_(rv.data, b'POST - API1')
    rv = client.delete('/admin1/mockmethodview/_api/1')
    eq_(rv.data, b'DELETE - API1')

    rv = client.get('/admin1/mockmethodview/_api/2')
    eq_(rv.data, b'GET - API2')
    rv = client.post('/admin1/mockmethodview/_api/2')
    eq_(rv.data, b'POST - API2')
    rv = client.delete('/admin1/mockmethodview/_api/2')
    eq_(rv.status_code, 405)
    rv = client.put('/admin1/mockmethodview/_api/2')
    eq_(rv.status_code, 405)

    rv = client.get('/admin1/mockmethodview/_api/3')
    eq_(rv.data, b'GET - API3')
    rv = client.get('/admin1/mockmethodview/_api/4')
    eq_(rv.data, b'GET - API3')
...
def check_endpoint():
    class CustomView(MockView):
        def _get_endpoint(self, endpoint):
            return 'admin1.' + super(CustomView, self)._get_endpoint(endpoint)

    view = CustomView()
    eq_(view.endpoint, 'admin1.customview')
