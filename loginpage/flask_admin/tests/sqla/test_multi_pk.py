...
def test_multiple_pk():
    ...
    rv = client.get('/admin1/model/edit/?id=1,two')
    eq_(rv.status_code, 200)
    data = rv.data.decode('utf-8')
    ok_('test3' in data)

    # Correct order is mandatory -> fail here
    rv = client.get('/admin1/model/edit/?id=two,1')
    eq_(rv.status_code, 302)
...
def test_joined_inheritance():
    ...
    rv = client.get('/admin1/child/')
    eq_(rv.status_code, 200)

    rv = client.post('/admin1/child/new/',
                     data=dict(id=1, test='foo', name='bar'))
    eq_(rv.status_code, 302)

    rv = client.get('/admin1/child/edit/?id=1')
    eq_(rv.status_code, 200)
    data = rv.data.decode('utf-8')
    ok_('foo' in data)
    ok_('bar' in data)
...
def test_concrete_multipk_inheritance():
    ...
    rv = client.get('/admin1/child/edit/?id=1,2')
    eq_(rv.status_code, 200)
    data = rv.data.decode('utf-8')
    ok_('foo' in data)
    ok_('bar' in data)
