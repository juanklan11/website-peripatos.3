...
def test_hstore():
    ...
    rv = client.post('/admin1/model/new/', data={
...
def test_json():
    ...
    rv = client.get('/admin1/jsonmodel/')
    eq_(rv.status_code, 200)

    rv = client.post('/admin1/jsonmodel/new/', data={
    ...
    rv = client.get('/admin1/jsonmodel/edit/?id=1')
    eq_(rv.status_code, 200)
    data = rv.data.decode('utf-8')
    ok_('json_test' in data)
    ok_('>{"test_key1": "test_value1"}<' in data)
...
def test_citext():
    ...
    rv = client.get('/admin1/citextmodel/')
    eq_(rv.status_code, 200)

    rv = client.post('/admin1/citextmodel/new/', data={
        'citext_test': 'Foo',
    })
    eq_(rv.status_code, 302)
    ...
    rv = client.get('/admin1/citextmodel/edit/?id=1')
    eq_(rv.status_code, 200)
    data = rv.data.decode('utf-8')
