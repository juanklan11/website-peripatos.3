...
def test_inline_form():
    ...
    rv = client.get('/admin1/user/')
    eq_(rv.status_code, 200)

    rv = client.get('/admin1/user/new/')
    eq_(rv.status_code, 200)

    # Create
    rv = client.post('/admin1/user/new/', data=dict(name=u'äõüxyz'))
    eq_(rv.status_code, 302)
    eq_(User.query.count(), 1)
    eq_(UserInfo.query.count(), 0)

    data = {'name': u'fbar', 'info-0-key': 'foo', 'info-0-val': 'bar'}
    rv = client.post('/admin1/user/new/', data=data)
    eq_(rv.status_code, 302)
    eq_(User.query.count(), 2)
    eq_(UserInfo.query.count(), 1)

    # Edit
    rv = client.get('/admin1/user/edit/?id=2')
    eq_(rv.status_code, 200)
    # Edit - update
    data = {
        'name': u'barfoo',
    ...
    rv = client.post('/admin1/user/edit/?id=2', data=data)
    eq_(UserInfo.query.count(), 1)
    ...
    # Delete
    rv = client.post('/admin1/user/delete/?id=2')
    eq_(rv.status_code, 302)
    eq_(User.query.count(), 1)
    rv = client.post('/admin1/user/delete/?id=1')
    eq_(rv.status_code, 302)
...
def test_inline_form_required():
    ...
    # Create
    rv = client.post('/admin1/user/new/', data=dict(name=u'no-email'))
    eq_(rv.status_code, 200)
    eq_(User.query.count(), 0)
