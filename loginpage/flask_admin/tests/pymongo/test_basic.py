...
def test_model():
    ...
    rv = client.get('/admin1/testview/')
    eq_(rv.status_code, 200)

    rv = client.get('/admin1/testview/new/')
    eq_(rv.status_code, 200)

    rv = client.post('/admin1/testview/new/',
    ...
    url = '/admin1/testview/edit/?id=%s' % model['_id']
    rv = client.get(url)
    eq_(rv.status_code, 200)
    ...
    url = '/admin1/testview/delete/?id=%s' % model['_id']
    rv = client.post(url)
    eq_(rv.status_code, 302)
    eq_(db.test.count(), 0)
