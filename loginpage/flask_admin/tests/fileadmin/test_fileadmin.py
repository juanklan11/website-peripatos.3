...
class Base:
    class FileAdminTests(unittest.TestCase):
        ...
        def test_file_admin(self):
            ...
            # index
            rv = client.get('/admin1/myfileadmin/')
            eq_(rv.status_code, 200)
            ok_('path=dummy.txt' in rv.data.decode('utf-8'))

            # edit
            rv = client.get('/admin1/myfileadmin/edit/?path=dummy.txt')
            eq_(rv.status_code, 200)
            ok_('dummy.txt' in rv.data.decode('utf-8'))

            rv = client.post('/admin1/myfileadmin/edit/?path=dummy.txt',
            ...
            # rename
            rv = client.get('/admin1/myfileadmin/rename/?path=dummy.txt')
            eq_(rv.status_code, 200)
            ok_('dummy.txt' in rv.data.decode('utf-8'))

            rv = client.post('/admin1/myfileadmin/rename/?path=dummy.txt',
            ...
            # upload
            rv = client.get('/admin1/myfileadmin/upload/')
            eq_(rv.status_code, 200)

            rv = client.post('/admin1/myfileadmin/upload/',
            ...
            # delete
            rv = client.post('/admin1/myfileadmin/delete/',
            ...
            # mkdir
            rv = client.get('/admin1/myfileadmin/mkdir/')
            eq_(rv.status_code, 200)

            rv = client.post('/admin1/myfileadmin/mkdir/',
            ...
            # rename - directory
            rv = client.get('/admin1/myfileadmin/rename/?path=dummy_dir')
            eq_(rv.status_code, 200)
            ok_('dummy_dir' in rv.data.decode('utf-8'))

            rv = client.post('/admin1/myfileadmin/rename/?path=dummy_dir',
...
class LocalFileAdminTests(Base.FileAdminTests):
    ...
    def test_fileadmin_sort_bogus_url_param(self):
        ...
        with open(op.join(self._test_files_root, 'dummy2.txt'), 'w') as fp:
            ...
            rv = client.get('/admin1/myfileadmin/?sort=bogus')
            eq_(rv.status_code, 200)
            ok_(rv.data.decode('utf-8').find('path=dummy2.txt') <
                rv.data.decode('utf-8').find('path=dummy.txt'))

            rv = client.get('/admin1/myfileadmin/?sort=name')
            eq_(rv.status_code, 200)
